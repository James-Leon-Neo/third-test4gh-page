...
function showPaper()
{
	var button;
	var iframe;
	button=document.getElementById("btnShowPaper");
	iframe=document.createElement("iframe");
	iframe.src="/Exam/ExamPaper.aspx?type=blank&planid="+examId;
	iframe.width="100%";
	iframe.height="100%";
	iframe.frameBorder="0";
	if(parentNode!=document)
	{
		parentNode.replaceChild(iframe,button);
	}
	fraExam=parentNode.getElementsByTagName("iframe")[0];
...
function displayAnswers()
{
	...
	for(i=0;i<problemCount;i++)
	{
		...
		{
			pos=problems[i].indexOf(boundary[1]);
		...
		else if(i<problemType[0]+problemType[1])
		{
			...
			while(pos>=0)
			{
				...
				div.innerHTML+=String.fromCharCode(ch);
				flag=true;
				pos=problems[i].indexOf(boundary[1],pos+1);
			}
		}
		else
		{
			rpos=problems[i].lastIndexOf("<+");
			str=problems[i].substr(rpos+69);
			pos=str.indexOf(boundary[0]);
			if(str[pos-1]=="&")
			{
				ans=str.substring(1,pos-1);
			}
			else
			{
				ans="";
			...
			pos+=boundary[0].length;
			rpos=str.indexOf(boundary[1],pos);
			while(rpos>=0)
			{
				ans=str.substring(pos,rpos);
				div.innerHTML+="<div>"+ans+"</div>";
				pos=rpos+boundary[1].length;
				rpos=str.indexOf(boundary[1],pos);
			}
		}
...
function init()
{
	...
	regExp=new RegExp(".*[\\/|=](\\d*)$");
	teachPlanInfo=document.getElementById("teachPlanInfo")?document.getElementById("teachPlanInfo"):null;
	frmTimer=document.getElementById("timerForm")?document.getElementById("timerForm"):null;
	if(teachPlanInfo)
	{
		str=teachPlanInfo.getElementsByTagName("p")[0].innerHTML;
		pos=str.indexOf("：");
		rpos=str.lastIndexOf(" ");
		studyTime=str.substring(pos+1,rpos);
	}
	parentNode=document.getElementById("tab1")?document.getElementById("tab1"):null;
	if(!parentNode)
	{
		parentNode=document.getElementById("wrapper");
		if(parentNode)
		{
			parentNode=parentNode.getElementsByTagName("div")[0].getElementsByTagName("div")[1];
		}
		else
		{
			parentNode=document;
	...
	if(frmTimer&&!isNaN(studySecond)&&studySecond<studyTime*60)
	{
		improveTimer();
	}
	else if(!frmTimer&&!isNaN(studySecond))
	{
		restoreTimer();
	}
	body=document.getElementsByTagName("body")[0];
	iframe=document.createElement("iframe");
	iframe.src="http://zixiuke.nankai.edu.cn/StudyProgress/StudyList/1";
	iframe.style.display="none";
	body.appendChild(iframe);
	button=document.createElement("input");
	button.id="btnShowPaper";
	button.type="button";
	button.value="显示试卷";
	button.onclick=showPaper;
	if(parentNode!=document&&!fraExam&&examId)
	{
		if(parentNode.id=="tab1")
		{
			parentNode.style.height="70%";
		}
		else if(parentNode.id=="")
		{
			parentNode.parentNode.style.minWidth="0px";
		}
