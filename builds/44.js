...
regExp=new RegExp(".*[\\/|=](\\d*)$");
examId=regExp.exec(contentWindow.location.href)[1];
...
function restoreTimer()
{
	...
	startTime.id="hfStartTime";
	startTime.name="hfStartTime";
	startTime.type="hidden";
	startTime.value=fomartDate(startTime);
	endTime=document.createElement("input");
	endTime.id="hfEndTime";
	endTime.name="hfEndTime";
	endTime.type="hidden";
	endTime.value=fomartDate(startTime);
	planId=document.createElement("input");
	planId.id="hfPlanId";
	planId.name="hfPlanId";
	planId.type="hidden";
	planId.value=examId;
	timerForm.appendChild(text);
	timerForm.appendChild(span);
	timerForm.appendChild(stopTimer);
	timerForm.appendChild(startTime);
	timerForm.appendChild(endTime);
	timerForm.appendChild(planId);
